...
start_task_timer "calculateOwnerHops" "system" '{"algorithm":"hop_distance","target":"owner","owner_pubkey":"'$BRAINSTORM_OWNER_PUBKEY'","max_hops":12}'
...
log_structured "PROGRESS" "calculateOwnerHops" "system" '{"phase":"initialization","step":"reset_all_hops","description":"Setting all users to 999 hops"}'
sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER1"

log_structured "PROGRESS" "calculateOwnerHops" "system" '{"phase":"initialization","step":"set_owner_zero","description":"Setting owner to 0 hops","owner_pubkey":"'$BRAINSTORM_OWNER_PUBKEY'"}'
sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER2"

# Phase 2: Iterative hop calculation
log_structured "PROGRESS" "calculateOwnerHops" "system" '{"phase":"calculation","step":"start_iterations","description":"Beginning iterative hop distance calculation"}'
cypherResults=$(sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER3")
numUpdates="${cypherResults:11}"

log_structured "PROGRESS" "calculateOwnerHops" "system" '{"phase":"calculation","step":"initial_iteration","hop_level":1,"updates":'$numUpdates',"description":"Completed initial hop calculation"}'
...
end_task_timer "calculateOwnerHops" "system" '{"algorithm":"hop_distance","final_hop_level":'$final_hops',"total_iterations":'$numHops',"completion_reason":"'$completion_reason'","max_hops":12}'