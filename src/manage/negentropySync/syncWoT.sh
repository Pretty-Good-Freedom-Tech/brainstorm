...
while true; do
    ...
    if [ $num_new_events -lt $num_events_increment ]; then
        ...
        echo "$(date): Less than $num_events_increment events in $time_increment seconds, exiting syncWoT"
        echo "$(date): Less than $num_events_increment events in $time_increment seconds, exiting syncWoT" >> ${BRAINSTORM_LOG_DIR}/syncWoT.log
        # kill all sync_pid processes
        # kill ${sync_pid[@]}
        # only exit if syncWoT_active_time is more than 3 minutes
