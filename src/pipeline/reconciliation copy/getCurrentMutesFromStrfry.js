...
/**
 * Get all distinct pubkeys that have kind 10000 events
 */
async function getAllPubkeys() {
  try {
    log('Getting list of unique pubkeys with kind 10000 events...');
    
    // Use strfry scan to list authors with kind 10000 events
    const command = 'sudo strfry scan "{\\"kinds\\":[10000]}" --limit 0 | jq -r .pubkey | sort | uniq';
    ...
    log(`Found ${pubkeys.length} unique pubkeys with kind 10000 events`);
    return pubkeys;
  } catch (error) {
...
/**
 * Process a batch of pubkeys and extract their mutes relationships
 */
async function processPubkeyBatch(pubkeys, batchIndex, totalBatches) {
  try {
    ...
    const batchOutputFile = path.join(tempDir, `mutes_batch_${batchIndex}.json`);
    ...
    for (let i = 0; i < pubkeys.length; i++) {
      ...
      try {
        // Get the latest kind 10000 event for this pubkey
        const eventJson = execSync(`sudo strfry scan "{\\"kinds\\":[10000],\\"authors\\":[\\"${pubkey}\\"]}" --limit 1`).toString().trim();
        ...
        if (eventJson) {
          ...
          // Extract mutes from the tags (p tags)
          const mutes = {};
          if (event.tags && Array.isArray(event.tags)) {
            for (const tag of event.tags) {
              if (Array.isArray(tag) && tag[0] === 'p' && tag[1]) {
                const mutedPubkey = tag[1];
                mutes[mutedPubkey] = event.created_at;
              }
            }
          }
          
          // If there are mutes, write to the batch output
          if (Object.keys(mutes).length > 0) {
            ...
            batchOutputStream.write(`  "${pubkey}": ${JSON.stringify(mutes)}`);
          }
        }
      } catch (error) {
      ...
      if ((i + 1) % 100 === 0 || i === pubkeys.length - 1) {
        ...
        log(`Batch ${batchIndex} of ${totalBatches} mutes from strfry progress: ${progress}% (${i + 1}/${pubkeys.length} pubkeys)`);
      }
    }
...
async function mergeBatchFiles(batchFiles) {
  try {
    ...
    const outputFile = path.join(config.outputDir, 'currentMutes_strfry.json');
    ...
    for (let i = 0; i < batchFiles.length; i++) {
      ...
      try {
        ...
        // Parse the content to extract just the pubkeys and mutes
...
async function main() {
  try {
    ...
    log('Starting extraction of current MUTES relationships from Strfry');
    
    // Ensure output directory exists
    ensureOutputDirectory();
    
    // Get all pubkeys with kind 10000 events
    ...
    const mutesData = JSON.parse(outputContent);
    
    const userCount = Object.keys(mutesData).length;
    let mutesCount = 0;
    
    for (const pubkey in mutesData) {
      mutesCount += Object.keys(mutesData[pubkey]).length;
    }
    
    log(`Extracted MUTES relationships for ${userCount} users`);
    log(`Found a total of ${mutesCount} MUTES relationships`);
    
    // Log completion time
    const endTime = Date.now();
    const duration = (endTime - startTime) / 1000;
    log(`Completed extraction of Strfry mutes in ${duration.toFixed(2)} seconds`);
