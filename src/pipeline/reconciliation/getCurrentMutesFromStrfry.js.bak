...
/**
 * getCurrentMutesFromStrfry.js
 * 
 * Extracts current MUTES relationships from Strfry database
 * Part of the reconciliation pipeline for Neo4j database maintenance
 * 
 * This script queries the Strfry database for kind 10000 (mute list) events
 * and extracts the mutes relationships, writing them to a JSON file
...
/**
 * Get the total count of kind 10000 events in Strfry
 * @returns {number} Total count of kind 10000 events
 */
async function getKind10000EventCount() {
  try {
    // Use strfry-query to count kind 10000 events
    const command = `cd ${config.strfryDir} && ./strfry-query --count 'SELECT * FROM events WHERE kind = 10000'`;
    const output = execSync(command).toString().trim();
    const count = parseInt(output, 10);
    return count;
  } catch (error) {
    await log(`ERROR: Failed to get kind 10000 event count: ${error.message}`);
    throw error;
  }
}

/**
 * Process a batch of kind 10000 events and extract mutes relationships
...
async function processEventBatch(offset, limit) {
  try {
    ...
    const tempFiles = {
      ...
      mutes: path.join(tempDir, `mutes_${batchId}.json`)
    };
    
    // Use strfry-query to get kind 10000 events for this batch
    const command = `cd ${config.strfryDir} && ./strfry-query 'SELECT * FROM events WHERE kind = 10000 ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}'`;
    const output = execSync(command).toString().trim();
    ...
    for (const event of events) {
      ...
      // Parse the tags to find p tags (mutes)
      if (event.tags && Array.isArray(event.tags)) {
        // Initialize mutes map for this user if it doesn't exist
        if (!mutes[pubkey]) {
          mutes[pubkey] = {};
        }
        
        // Extract p tags (mutes)
        for (const tag of event.tags) {
          if (Array.isArray(tag) && tag[0] === 'p' && tag[1]) {
            const targetPubkey = tag[1];
            // Store the timestamp as the value
            mutes[pubkey][targetPubkey] = event.created_at;
          }
        }
      }
    }
    ...
    await writeFile(tempFiles.mutes, JSON.stringify(mutes));
    
    return {
      stats: {
        eventsProcessed: events.length,
        usersWithMutes: Object.keys(mutes).length,
        totalMutes: Object.values(mutes).reduce((sum, muteMap) => sum + Object.keys(muteMap).length, 0)
      },
      tempFiles
    };
  } catch (error) {
 ...
 */
async function writeMutesData(tempFilesArray) {
  const strfryMutesFile = path.join(config.outputDir, 'currentMutes_strfry.json');
  ...
  const outputStream = fs.createWriteStream(strfryMutesFile);
  ...
  await log(`Wrote Strfry mutes data to ${strfryMutesFile}`);
  ...
  return { filePath: strfryMutesFile };
}
...
async function main() {
  try {
    ...
    await log('Starting extraction of current MUTES relationships from Strfry');
    
    // Ensure output directory exists
    await ensureOutputDirectory();
    
    // Get total event count for progress tracking
    const totalEvents = await getKind10000EventCount();
    await log(`Found ${totalEvents} kind 10000 events in Strfry`);
    
    // Process events in batches
    const tempFiles = [];
    let totalEventsProcessed = 0;
    let totalUsersWithMutes = 0;
    let totalMutesCount = 0;
    ...
    for (let offset = 0; offset < totalEvents; offset += config.batchSize) {
      ...
      totalUsersWithMutes += stats.usersWithMutes;
      totalMutesCount += stats.totalMutes;
    ...
    await log(`Processed ${totalEventsProcessed} kind 10000 events`);
    await log(`Found ${totalUsersWithMutes} users with MUTES relationships`);
    await log(`Found ${totalMutesCount} total MUTES relationships`);
    
    // Merge temporary files into final output
    await log('Merging batch files into final output...');
    await writeMutesData(tempFiles);
    
    // Log completion time
    const endTime = Date.now();
    const duration = (endTime - startTime) / 1000;
    await log(`Completed extraction of Strfry mutes in ${duration.toFixed(2)} seconds`);
