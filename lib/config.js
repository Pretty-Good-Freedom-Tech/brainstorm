...
function getConfigValue(varName) {
  try {
    if (fs.existsSync(PROD_CONFIG_PATH)) {
      const result = execSync(`source ${PROD_CONFIG_PATH} && echo $${varName}`).toString().trim();
      return result || null;
    }
    return null;
  } catch (error) {
