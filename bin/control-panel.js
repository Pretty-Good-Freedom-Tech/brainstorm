...
function handleGetInstanceStatus(req, res) {
    ...
    promises.push(
        new Promise((resolve) => {
            exec('systemctl is-active strfry', (error, stdout, stderr) => {
                result.strfry.service.status = (stdout.trim() === 'active') ? 'running' : 'stopped';
                resolve();
            });
        })
    );
        ...
        return new Promise((resolve) => {
            ...
            exec(cmd, (error, stdout, stderr) => {
                ...
                try {
                    ...
                    result.strfry.events.byKind[kind] = isNaN(count) ? 0 : count;
                } catch (e) {
        ...
        new Promise((resolve) => {
            ...
            exec(cmd, (error, stdout, stderr) => {
                ...
                try {
                    // Parse count from output
                    const count = parseInt(stdout.trim(), 10);
                    result.strfry.events.total = isNaN(count) ? 0 : count;
                } catch (e) {
        ...
        new Promise((resolve) => {
            ...
            exec(cmd, (error, stdout, stderr) => {
                ...
                try {
                    // Parse count from output
                    const count = parseInt(stdout.trim(), 10);
                    result.strfry.events.recent = isNaN(count) ? 0 : count;
                } catch (e) {
    ...
    // 5. Check Neo4j service status
    promises.push(
        new Promise((resolve) => {
            exec('systemctl is-active neo4j', (error, stdout, stderr) => {
                result.neo4j.service.status = (stdout.trim() === 'active') ? 'running' : 'stopped';
                resolve();
            });
        })
    );
    
    // 6. Get Neo4j connection details for Cypher queries
    const neo4jConnection = getNeo4jConnection();
    const neo4jUser = neo4jConnection.user;
    const neo4jPassword = neo4jConnection.password;
    ...
    if (neo4jUser && neo4jPassword) {
        // 7. Check Neo4j constraints and indexes
        promises.push(
            new Promise((resolve) => {
                ...
                exec(command, (error, stdout, stderr) => {
                    if (error) {
                        result.neo4j.constraints.status = 'error';
                        resolve();
                        return;
                    }
                    ...
                    if (stdout.includes('UNIQUENESS') && stdout.includes('NostrUser') && stdout.includes('pubkey')) {
                        result.neo4j.constraints.status = 'ok';
                    } else {
                        result.neo4j.constraints.status = 'missing';
                    }
                    resolve();
                });
            })
        ...
        promises.push(
            new Promise((resolve) => {
                ...
                exec(command, (error, stdout, stderr) => {
                    if (error) {
                        result.neo4j.indexes.status = 'error';
                        resolve();
                        return;
                    }
                    ...
                    if (stdout.includes('INDEX')) {
                        result.neo4j.indexes.status = 'ok';
                    } else {
                        result.neo4j.indexes.status = 'missing';
                    }
                    resolve();
                });
            })
            ...
            new Promise((resolve) => {
                ...
                exec(command, (error, stdout, stderr) => {
                    ...
                    try {
                        ...
                        if (match && match[1]) {
                            result.neo4j.users.total = parseInt(match[1], 10);
                        }
                    } catch (e) {
            ...
            new Promise((resolve) => {
                ...
                exec(command, (error, stdout, stderr) => {
                    ...
                    try {
                        ...
                        const match = stdout.match(/(\d+)/);
                        if (match && match[1]) {
                            result.neo4j.relationships.total = parseInt(match[1], 10);
                        }
                    } catch (e) {
                ...
                new Promise((resolve) => {
                    ...
                    exec(command, (error, stdout, stderr) => {
                        ...
                        try {
                            ...
                            const match = stdout.match(/(\d+)/);
                            if (match && match[1]) {
                                result.neo4j.relationships[relType.toLowerCase()] = parseInt(match[1], 10);
                            }
                        } catch (e) {
        ...
        // 11. Count recent relationships (past hour)
        promises.push(
            new Promise((resolve) => {
                const oneHourAgo = Math.floor(Date.now() / 1000) - 3600;
                ...
                exec(command, (error, stdout, stderr) => {
                    ...
                    try {
                        ...
                        const match = stdout.match(/(\d+)/);
                        if (match && match[1]) {
                            result.neo4j.relationships.recent = parseInt(match[1], 10);
                        }
                    } catch (e) {
        ...
        // 12. Count verified users (GrapeRank influence > 0.05)
        promises.push(
            new Promise((resolve) => {
                ...
                new Promise((resolve) => {
                    ...
                    exec(command, (error, stdout, stderr) => {
                        ...
                        try {
                            ...
                            const match = stdout.match(/(\d+)/);
                            if (match && match[1]) {
                                result.followsNetwork.byHops[hops] = parseInt(match[1], 10);
                            }
                        } catch (e) {
        ...
        // 14. Count total users in follows network (hops < 20)
        promises.push(
            new Promise((resolve) => {
                ...
                exec(command, (error, stdout, stderr) => {
                    ...
                    try {
                        ...
                        const match = stdout.match(/(\d+)/);
                        if (match && match[1]) {
                            result.followsNetwork.total = parseInt(match[1], 10);
                        }
                    } catch (e) {
    ...
    // 15. Check whitelist count and last updated
    promises.push(
        new Promise((resolve) => {
    ...
    // 16. Check blacklist count and last updated
    promises.push(
        new Promise((resolve) => {
    ...
    // 17. Check GrapeRank last updated
    promises.push(
        new Promise((resolve) => {
    ...
    // 18. Check PageRank last updated
    promises.push(
        new Promise((resolve) => {
    ...
    // 19. Check Follows Network last calculated
    promises.push(
        new Promise((resolve) => {
            ...
            fs.access(hopsLogPath, fs.constants.F_OK, (err) => {
                ...
                fs.stat(hopsLogPath, (statErr, stats) => {
                    ...
                    result.followsNetwork.lastCalculated = Math.floor(stats.mtime.getTime() / 1000);
                    resolve();
                });
            });
        })
    ...
    Promise.all(promises)
        .then(() => {
            console.log('Instance status data collected successfully');
            res.json(result);
        })
        .catch(error => {
            console.error('Error collecting instance status data:', error);
